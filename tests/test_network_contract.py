...
This file is the **pre-conversion recording**: every value below is what the
unmodified trunk serves today, including the two things the conventions pass
in this PR will deliberately change (the dict-valued ``detail`` payloads and
the ``{"status": "ok"}`` delete envelope). Recording them first is what makes
the later diff reviewable — a golden written after the change proves nothing.

Seam targets
------------
The WiFi service singleton is ``src.network.wifi.get_wifi_service``, which the
extraction does not move, so no ``patch()`` target in this file changes when
the handlers leave ``src/api_server.py``.
"""
...
def test_the_refusal_falls_back_to_a_sentence_when_the_service_gives_no_reason(client):
    ...
    assert resp.json()["detail"] == {
        "status": "unavailable",
        "reason": "WiFi management is unavailable on this deployment.",
    }
...
def test_status_reports_a_wifi_error_as_400_carrying_the_message(client, available):
    ...
    assert resp.json()["detail"] == {"status": "error", "error": "nmcli broken"}
...
def test_scan_reports_a_wifi_error_as_400(client, available):
    with patch.object(get_wifi_service(), "scan", side_effect=WiFiError("no wireless device")):
        resp = client.post("/network/wifi/scan")
    assert resp.status_code == 400
    assert resp.json()["detail"] == {"status": "error", "error": "no wireless device"}
...
def test_saved_returns_the_stored_profiles(client, available):
    with patch.object(get_wifi_service(), "saved_networks", return_value=[SavedNetwork(name="HomeNet", autoconnect=True)]):
...
def test_forget_reports_a_wifi_error_as_400(client, available):
    ...
    assert resp.status_code == 400
    assert resp.json()["detail"] == {"status": "error", "error": "profile not found"}
...
def test_connect_reports_a_wifi_error_as_400(client, available):
    ...
    assert resp.status_code == 400
    assert resp.json()["detail"] == {"status": "error", "error": "auth failed"}
...
def test_disconnect_reports_a_wifi_error_as_400(client, available):
    ...
    with patch.object(get_wifi_service(), "disconnect", side_effect=_boom):
        resp = client.post("/network/wifi/disconnect")
    assert resp.status_code == 400
    assert resp.json()["detail"] == {"status": "error", "error": "nmcli down failed"}
