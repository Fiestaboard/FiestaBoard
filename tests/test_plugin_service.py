...
only sanctioned reach into ``ConfigManager._mask_sensitive`` (the update-status
cache is cleared through the registry's own locked accessor, #1828). These
tests pin the sequence and the error
...
def _service(recorder: _Recorder, *, config_errors: list[str] | None = None) -> PluginService:
    ...
    # The real registry pops under its own lock (#1828); mimic that here so
    # the service's delegation is observable end-to-end.
...
def test_clear_update_status_goes_through_the_registrys_locked_accessor():
    """The cache must be popped under the registry lock (#1828), so the
    service delegates instead of reaching into ``_update_status`` itself."""
    svc = _service(_Recorder())
    svc.registry._update_status = {"ext": True}

    svc.clear_update_status("ext")

    svc.registry.clear_update_status.assert_called_once_with("ext")
