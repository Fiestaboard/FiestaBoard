...
let capturedHandlers: Parameters<typeof import("@/lib/api-stream")["streamChat"]>[1] | null = null;
...
describe("useAiChat", () => {
  ...
  it("onText handler appends delta to the in-flight assistant message", async () => {
    const { result } = renderHook(() => useAiChat(makeOpts()));
    act(() => { result.current.send("hi"); });
    await act(async () => { capturedHandlers?.onText?.("hello "); });
    await act(async () => { capturedHandlers?.onText?.("world"); });
    const last = result.current.messages[result.current.messages.length - 1];
  ...
  it("onToolCall attaches tool call to the assistant message and fires callback", async () => {
    ...
    act(() => { result.current.send("hi"); });
    const call: ToolCall = { id: "tc1", op: "replace_page", args: { name: "p", template: [], line_metadata: [], duration_seconds: 300 } };
    await act(async () => { capturedHandlers?.onToolCall?.(call); });
    const last = result.current.messages[result.current.messages.length - 1];
  ...
  it("onWarning attaches warning to the assistant message", async () => {
    const { result } = renderHook(() => useAiChat(makeOpts()));
    act(() => { result.current.send("hi"); });
    await act(async () => { capturedHandlers?.onWarning?.("rate limited"); });
    const last = result.current.messages[result.current.messages.length - 1];
  ...
  it("onError sets the error state", async () => {
    const { result } = renderHook(() => useAiChat(makeOpts()));
    act(() => { result.current.send("hi"); });
    await act(async () => { capturedHandlers?.onError?.("oops"); });
  ...
  it("status is 'error' after stream with an error", async () => {
    const { result } = renderHook(() => useAiChat(makeOpts()));
    act(() => { result.current.send("hi"); });
    await act(async () => { capturedHandlers?.onError?.("fail"); });
    await act(async () => { resolveStream?.(); });
  ...
  it("reset() clears messages, status, and error", async () => {
    const { result } = renderHook(() => useAiChat(makeOpts()));
    act(() => { result.current.send("hi"); });
    await act(async () => { capturedHandlers?.onError?.("bad"); resolveStream?.(); });
