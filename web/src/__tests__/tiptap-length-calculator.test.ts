...
import {
  ...
  getOverflowAmount,
  willOverflow,
} from "../components/tiptap-template-editor/utils/length-calculator";

describe("Length Calculator", () => {
  describe("calculateLineLength", () => {
    it("calculates plain text length", () => {
      const content: JSONContent[] = [{ type: "text", text: "Hello World" }];
  ...
  describe("willOverflow", () => {
    it("detects no overflow", () => {
      const content: JSONContent[] = [{ type: "text", text: "Short" }];
