...
describe("PageBuilder — Gen AI button", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it("renders the Gen AI button and disables it when AI is not configured", async () => {
    render(<PageBuilder onClose={vi.fn()} />, { wrapper: TestWrapper });

    const button = await screen.findByRole("button", { name: "Gen AI" });
    expect(button).toBeInTheDocument();
    // The default mock has enabled: false / no providers, so the button
    // is disabled.
    await waitFor(() => {
  ...
  it("enables the Gen AI button when at least one provider is configured", async () => {
    server.use(
