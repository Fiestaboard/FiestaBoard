 ...
 * is the FiestaPanel TV viewer, which must never grow chrome or bounce an
 * unauthenticated TV browser to the login form.
 */
const CHROMELESS_PREFIXES = ["/login", "/panel/"] as const;
...
/** True on the FiestaPanel viewer route (`/panel/:panelId`). */
export function isPanelPath(pathname: string): boolean {
  return normalize(pathname).startsWith("/panel/");
}
