...
- [ ] **Step 2: Job gate** — `if:` gates only on `startsWith(github.event.pull_request.head.ref, 'bug-hunt/issue-')`. Do NOT add a `draft` condition: because triage opens these as DRAFT, the reviewer must fire on draft PRs (unlike the a11y/docs reviewers, which skip drafts). Add a plain comment saying draft bug-hunt PRs are reviewed intentionally, rather than a `draft == false || true` tautology.
